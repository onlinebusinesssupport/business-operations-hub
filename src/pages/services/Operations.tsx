...
import { ArrowLeft, Check, ArrowRight, Layers, Calendar, Mail, Users, Plane, BarChart3 } from "lucide-react";
...
const whatWeDo = [
  { icon: Mail, text: "Inbox management" },
  { icon: Calendar, text: "Calendar management" },
  { icon: Users, text: "Client communication" },
  { icon: Plane, text: "Travel management & bookings" },
  { icon: BarChart3, text: "Vendor coordination & CRM updates" },
  { icon: Layers, text: "Research support & founder operations" },
];
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-6 lg:px-8 max-w-3xl text-center">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="mt-8 p-8 bg-card rounded-xl border border-border/50 shadow-lg">
            <p className="text-sm text-muted-foreground leading-relaxed max-w-xl mx-auto">
              We custom quote all retainer services based on your specific needs.
            </p>
            <div className="mt-6 flex flex-col sm:flex-row gap-4 justify-center items-center">
              <div className="text-center">
                <p className="text-xs text-muted-foreground uppercase tracking-wider">VA Retainers from</p>
                <p className="mt-1 text-2xl font-bold text-foreground">R12,000<span className="text-sm font-normal text-muted-foreground">/mo</span></p>
              </div>
              <div className="hidden sm:block h-12 w-px bg-border" />
              <div className="text-center">
                <p className="text-xs text-muted-foreground uppercase tracking-wider">Operations from</p>
                <p className="mt-1 text-2xl font-bold text-foreground">R24,000<span className="text-sm font-normal text-muted-foreground">/mo</span></p>
              </div>
            </div>
          </motion.div>
          <div className="mt-8">
      ...
      <section className="py-24 bg-primary text-primary-foreground">
        <div className="container mx-auto px-6 lg:px-8 text-center max-w-2xl">
          <motion.h2 initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="font-display text-2xl md:text-3xl font-bold uppercase tracking-tight">Activate Operations</motion.h2>
          <motion.p initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="mt-4 text-sm text-primary-foreground/70">Reclaim your time. Get matched with a dedicated operations specialist.</motion.p>
