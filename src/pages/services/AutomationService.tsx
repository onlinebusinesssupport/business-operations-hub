...
import { ArrowLeft, Check, ArrowRight, Zap, GitBranch, Link2, FileText, Mail } from "lucide-react";
...
const whatWeDo = [
  { icon: GitBranch, text: "Workflow mapping & design" },
  { icon: Link2, text: "CRM automation & integrations" },
  { icon: Mail, text: "Lead nurturing sequences" },
  { icon: FileText, text: "SOP documentation" },
  { icon: Zap, text: "Tool integration between platforms" },
];
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-36 pb-20">
        <div className="container mx-auto px-6 lg:px-8 max-w-3xl">
          ...
          <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1, ease: easeOut }} className="mt-6 text-base md:text-lg text-muted-foreground leading-relaxed max-w-lg">
            Time is your most expensive bottleneck. Let's eliminate it.
          </motion.p>
        ...
        <div className="container mx-auto px-6 lg:px-8 max-w-3xl text-center">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="mt-8 p-8 bg-card rounded-xl border border-border/50 shadow-lg">
            <p className="text-sm text-muted-foreground leading-relaxed max-w-xl mx-auto">
              We custom quote all automation projects based on complexity and scope.
            </p>
            <div className="mt-6 flex flex-col sm:flex-row gap-4 justify-center items-center">
              <div className="text-center">
                <p className="text-xs text-muted-foreground uppercase tracking-wider">Once-off builds from</p>
                <p className="mt-1 text-2xl font-bold text-foreground">R7,500</p>
              </div>
              <div className="hidden sm:block h-12 w-px bg-border" />
              <div className="text-center">
                <p className="text-xs text-muted-foreground uppercase tracking-wider">Retainers from</p>
                <p className="mt-1 text-2xl font-bold text-foreground">R6,000<span className="text-sm font-normal text-muted-foreground">/mo</span></p>
              </div>
            </div>
          </motion.div>
          <div className="mt-8">
      ...
      <section className="py-24 bg-primary text-primary-foreground">
        <div className="container mx-auto px-6 lg:px-8 text-center max-w-2xl">
          <motion.h2 initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="font-display text-2xl md:text-3xl font-bold uppercase tracking-tight">Automate My Business</motion.h2>
          <motion.p initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="mt-4 text-sm text-primary-foreground/70">Tell us what slows you down. We'll build the system to fix it.</motion.p>
