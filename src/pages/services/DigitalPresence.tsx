...
import { ArrowLeft, Check, ArrowRight, Globe, Instagram, Linkedin, PenTool, BarChart3, MessageCircle } from "lucide-react";
...
const whatWeDo = [
  { icon: PenTool, text: "Platform strategy & content planning" },
  { icon: MessageCircle, text: "Caption writing & design" },
  { icon: Instagram, text: "Engagement management" },
  { icon: BarChart3, text: "Performance tracking & reporting" },
  { icon: Linkedin, text: "LinkedIn & professional presence" },
];
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-6 lg:px-8 max-w-3xl">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: easeOut }} className="flex items-center gap-3 mb-6">
            <div className="p-2.5 rounded-lg bg-primary/10">
              <Globe size={22} className="text-primary" strokeWidth={1.5} />
            </div>
        ...
        <div className="container mx-auto px-6 lg:px-8 max-w-3xl text-center">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="mt-8 p-8 bg-card rounded-xl border border-border/50 shadow-lg">
            <p className="text-sm text-muted-foreground leading-relaxed max-w-xl mx-auto">
              We custom quote all Digital Presence retainers based on platforms, posting frequency, and content complexity.
            </p>
            <div className="mt-6">
              <p className="text-xs text-muted-foreground uppercase tracking-wider">Retainers from</p>
              <p className="mt-1 text-3xl font-bold text-foreground">R7,500<span className="text-sm font-normal text-muted-foreground">/mo</span></p>
            </div>
          </motion.div>
          <div className="mt-8">
            <Link to="/contact">
      ...
      <section className="py-24 bg-primary text-primary-foreground">
        <div className="container mx-auto px-6 lg:px-8 text-center max-w-2xl">
          <motion.h2 initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="font-display text-2xl md:text-3xl font-bold uppercase tracking-tight">Activate Digital Presence</motion.h2>
          <motion.p initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="mt-4 text-sm text-primary-foreground/70">Your brand deserves more than silence. Let's build a presence that compounds.</motion.p>
